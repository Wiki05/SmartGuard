...
/* ─── Wallet Connect Modal — rendered via Portal to <body> ─────── */
function WalletModal({ onClose }) {
  const [connecting, setConnecting] = useState(null);
  const [connectedLocally, setConnectedLocally] = useState(null);
  const { address, connectMetaMask, disconnect } = useWallet();
  ...
  const connect = async (wallet) => {
    ...
    if (wallet.id === "metamask") {
      try {
        ...
        setConnectedLocally({ ...wallet, address: addr });
        setConnecting(null);
        return;
      } catch (e) {
        console.error(e);
    ...
    setConnectedLocally({ ...wallet, address: fake });
    setConnecting(null);
  };

  const connectedState = address && connectedLocally?.id === "metamask" ? 
    { ...connectedLocally, address } : connectedLocally;

  const handleDisconnect = () => {
    if (connectedState?.id === 'metamask') {
      disconnect();
    }
    setConnectedLocally(null);
  }
    ...
    >
      <div
        ...
        style={{
          ...
          padding: "2rem",
          boxShadow: "0 40px 100px rgba(0,0,0,0.98), 0 0 0 1px rgba(168,255,108,0.04)",
      ...
      >
        ...
        {!connectedState ? (
          <>
            <div style={{ marginBottom: "1.5rem", paddingRight: 36 }}>
              <h2 style={{ fontWeight: 800, fontSize: 20, color: "#fff", marginBottom: 4 }}>Connect Wallet</h2>
              <p style={{ fontSize: 13, color: "#555" }}>Choose a wallet to connect to SmartGuard</p>
            </div>

            <div style={{ fontSize: 10, fontWeight: 700, color: "#333", letterSpacing: 2, textTransform: "uppercase", marginBottom: 10 }}>
              Popular Wallets
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {WALLETS.map(w => (
                <button key={w.id} onClick={() => connect(w)} disabled={!!connecting}
                  style={{
                    display: "flex", alignItems: "center", gap: 14,
                    padding: "0.9rem 1.1rem",
                    background: connecting === w.id ? `${w.color}18` : "#111",
                    border: `1px solid ${connecting === w.id ? w.color + "60" : "#1e1e1e"}`,
                    borderRadius: 14, cursor: connecting ? "wait" : "pointer",
                    transition: "all 0.18s", textAlign: "left", width: "100%",
                    fontFamily: "var(--font)",
                  }}
                  onMouseEnter={e => { if (!connecting) { e.currentTarget.style.background = `${w.color}12`; e.currentTarget.style.borderColor = w.color + "50"; e.currentTarget.style.transform = "translateX(3px)"; }}}
                  onMouseLeave={e => { e.currentTarget.style.background = "#111"; e.currentTarget.style.borderColor = "#1e1e1e"; e.currentTarget.style.transform = "translateX(0)"; }}
                >
                  {/* Real wallet icon */}
                  <div style={{ width: 36, height: 36, borderRadius: 10, overflow: "hidden", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "#1a1a1a" }}>
                    {WalletIcons[w.icon]}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 700, color: "#e8e8e8", marginBottom: 2 }}>{w.name}</div>
                    <div style={{ fontSize: 12, color: "#444" }}>{w.desc}</div>
                  </div>
                  {connecting === w.id ? (
                    <div style={{ width: 18, height: 18, borderRadius: "50%", border: `2px solid ${w.color}40`, borderTopColor: w.color, animation: "spin 0.8s linear infinite", flexShrink: 0 }} />
                  ) : (
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M6 3l5 5-5 5" stroke="#333" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  )}
                </button>
              ))}
            </div>
            <p style={{ fontSize: 11, color: "#282828", textAlign: "center", marginTop: "1.2rem" }}>
          ...
          <div style={{ textAlign: "center", padding: "1rem 0" }}>
            <div style={{ width: 64, height: 64, borderRadius: 18, overflow: "hidden", margin: "0 auto 1rem", display: "flex", alignItems: "center", justifyContent: "center", background: "#1a1a1a" }}>
              {WalletIcons[connectedState.icon || 'metamask']}
            </div>
            <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(168,255,108,0.1)", border: "2px solid rgba(168,255,108,0.4)", display: "flex", alignItems: "center", justifyContent: "center", margin: "-20px auto 16px", position: "relative" }}>
              <span style={{ fontSize: 22 }}>✓</span>
            </div>
            <h3 style={{ fontSize: 18, fontWeight: 800, color: "#a8ff6c", marginBottom: 8 }}>{connectedState.name || 'MetaMask'} Connected!</h3>
            <div style={{ fontSize: 13, color: "#555", fontFamily: "monospace", background: "#111", padding: "8px 16px", borderRadius: 10, display: "inline-block", marginBottom: 20, border: "1px solid #1e1e1e" }}>
              {connectedState.address}
            </div>
            <br/>
            <button onClick={onClose} style={{
              background: "linear-gradient(135deg, #a8ff6c, #00e5ff)",
              color: "#000", border: "none", padding: "0.8rem 2.5rem",
              borderRadius: 50, fontWeight: 800, fontSize: 15, cursor: "pointer",
              boxShadow: "0 0 25px rgba(168,255,108,0.5)",
              fontFamily: "var(--font)",
            }}>
              Continue to SmartGuard →
            </button>
          </div>
        )}
      </div>
...
function WalletButton() {
  ...
  const formattedAddress = connected ? address.slice(0, 6) + "…" + address.slice(-4) : "";

  return (
    <>
      <button onClick={() => connected ? disconnect() : setShowModal(true)} style={{
        display: "flex", alignItems: "center", gap: 7,
        padding: "0.4rem 1.1rem",
        background: connected
          ? "rgba(168,255,108,0.1)"
          : "linear-gradient(135deg, rgba(168,255,108,0.15), rgba(0,229,255,0.08))",
        border: `1px solid ${connected ? "rgba(168,255,108,0.4)" : "rgba(168,255,108,0.35)"}`,
        borderRadius: 50, fontSize: 12.5, fontWeight: 700,
        color: connected ? "#a8ff6c" : "#a8ff6c",
